...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create CBT-2fef
  fly apps create CBT-2fef-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app CBT-2fef
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app CBT-2fef-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'CBT-2fef-staging' does not match app name in config file 'CBT-2fef'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name CBT-2fef-db
  fly postgres attach --app CBT-2fef CBT-2fef-db

  fly postgres create --name CBT-2fef-staging-db
  fly postgres attach --app CBT-2fef-staging CBT-2fef-staging-db
  ```
